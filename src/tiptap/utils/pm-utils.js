...
export const DragNode = function ({
  ...
  if (!draggedNode || !targetNode) {
    console.error("Invalid nodes encountered during drag operation");
    return; // Avoid proceeding if nodes are invalid
  }
...
export const MoveNode = function ({ view, dir, currentResolved }) {
  ...
  // Ensure currentNode exists
  if (!currentNode) {
    console.error("Current node is invalid or not resolved.");
    return false; // Exit if current node is not valid
  }
  ...
  if (swapWithNodeSize === undefined) {
    console.error("swapWithNodeSize is undefined");
    return false;
  }
